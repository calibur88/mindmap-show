...
describe('曲线模式 · 安全推力', () => {
  it('正对直连强制 ≥ 25px 鼓起，且两控制点同向偏移（单向弧而非 S 形）', () => {
    // spread = 0 → h 由 5 抬到 25；此时两侧没有「左右」之分，第二控制点与首控制点同向 → 75 / 75
    expect(path(node(0, 0), node(0, 100), 'TB', 'curve')).toBe('M 50 40 C 75 70 75 70 50 100');
  });

  it('正对直连在水平布局下同样同向偏移', () => {
    // 中线 y = 20，两控制点都 +25
    expect(path(node(0, 0), node(200, 0), 'LR', 'curve')).toBe('M 100 20 C 150 45 150 45 200 20');
  });
