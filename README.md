...
> **文档版本**：v1.2（2026-09-09）· 插件版本：1.1.0
> **版本兼容性**：1.1.0与1.0.0完全兼容，`.mms`语法v1自本版起固定。此前无已发布版本，不存在配置迁移，既有`data.json`设置可沿用。
...
2. **画布（中间）**：探索／全景一键切换，鼠标拖拽与滚轮缩放、触控单指平移与双指缩放共用同一视口控件，缩放范围0.1~8，选中节点高亮并与右栏联动；
3. **节点详情（右栏）**：展示来源文件、节点正文、节点注释、嵌入资源、标签、引用链（该节点的`<=>`跨边＋`::`定位）、入链（`<=>`指向该节点的来源，同文件显示为「本文件」）、出链（文件级外部`<=>`聚合），未选中节点时给出占位提示，文件移动后给出降级提示；
4. **刷新流水线**：全库扫描→解析→跨文件引用解析→出链聚合，一次刷新让三个视图同步；`.mms`文件变动后自动防抖触发；
5. **设置（9项）**：默认视图、调试信息开关、3个线宽、4个间距；改动后400毫秒防抖自动落盘、重绘并重新扫描，无需手动刷新；
...
│  └─ utils/               DOM工具与键值构造
├─ demo/                   入库示例库（15个.mms分6目录，覆盖全部语法）
├─ docs/                   .mms语言规范
├─ styles.css              全部样式集中一处
├─ manifest.json           插件清单（版本1.1.0）
