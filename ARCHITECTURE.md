...
> 本文是Mind Map Show（MMS）工程的权威架构说明。文档版本：v1.5（2026-09-10）
> 插件版本：1.7.1 · 语法版本：`.mms` v1.4 · 最低依赖：Obsidian1.4.0 · 语言：TypeScript5.7（严格模式）
...
画布点击节点 → MmsSelection.set(文件路径, 节点id) → 订阅者回调
   ...
   （右栏「跳转」反向选中时同样触发画布高亮）
画布工具栏节点搜索（检索节点名称/正文/注释/引用与嵌入等全部展示文本，大小写不敏感，
   精确名称命中优先、其余按源码行序；节点id是内部标识不参与匹配）
   → 命中走同一条选中链路 + CanvasViewport.centerOnElement 平移居中（缩放不变）；
   多个命中（如同名/重复文本节点）用「上一个/下一个」循环导航＋计数显示；
   节点处于折叠分支（被剪枝不在DOM）时只选中不居中，footer提示；
   「清空」撤销搜索词与命中态、隐藏导航并回退默认态（无搜索词时忽略，不触发检索）
...
```
点击「导出图片」→ StatusCard 调 exportFlow.requestExport()（main注入）
   → resolveExportTarget 三级探测当前文件（最近leaf的FileView.file → getActiveFile → selection兜底）
   → index.getDoc(filePath) → buildExportSvg（复用全景渲染器＋内联样式＋xmlns）
   → 展开保存栏（默认路径 export/<源文件名>.svg）
确认 → StatusCard 调 exportFlow.confirmSave(path, svg)（main实现）
   → normalizePath + 补 .svg + 逐层 createFolder + 覆盖Modal
   → vault.create / vault.modify → new Notice；失败 reject 由状态卡行内显示
```

画布工具栏（两行布局，安卓适配）：上行文件名＋标签区（超3个折叠为「+N」＋展开/收起按钮，默认收起防溢出，切换文件重置）；下行节点搜索框＋视图按钮（重置视图/探索/全景）。搜索词与焦点在重渲染时保留（rescan不打断输入）；「清空」按钮撤销搜索词并清除命中态（无搜索词时忽略，不触发检索，与左栏搜索清空语义一致）。
