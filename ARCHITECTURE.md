...
> 插件版本：1.7.0 · 语法版本：`.mms` v1.4 · 最低依赖：Obsidian1.4.0 · 语言：TypeScript5.7（严格模式）
...
画布工具栏（两行布局，安卓适配）：上行文件名＋标签区（超3个折叠为「+N」＋展开/收起按钮，默认收起防溢出，切换文件重置）；下行节点搜索框＋视图按钮（重置视图/探索/全景）。搜索词与焦点在重渲染时保留（rescan不打断输入）。
