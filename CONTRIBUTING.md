...
> 同一文件禁止并行调用 Edit：每个 Edit 都读-改-写整文件，后写会静默覆盖先写（lost update），批量修改请改用脚本顺序执行并以 git diff 核验工作区。
...
- 版本号已确定但功能尚未推送时，仅[CHANGELOG.md](CHANGELOG.md)条目标题标记`未推送`（不写日期），`manifest.json`与`package.json`暂不升版，待正式推送时三处同步；
...
| `CHANGELOG.md` | 版本条目（功能更新时按§6.1编写并标记`未推送`／`已推送`状态，不得遗漏） |
...
> 功能在已确定版本号上编写完成后，先在更新日志里标记该版本条目与**状态**（未推送／已推送）；正式推送时再取消未推送标记、补写日期。
> CHANGELOG不随每次提交自动追加，但功能更新时**必须**按§6.1在已确定版本号上编写条目并标记状态，不得遗漏。
...
---
## [X.Y.Z] - YYYY-MM-DD（当前）
（正式推送：补写发布日期、取消「未推送」标记、标注（当前），并将版本号同步到manifest.json与package.json）
---
## [特殊条目] - YYYY-MM-DD
...
| 未推送状态 | 版本号已确定、条目已编写但功能尚未推送时，标题写`## [X.Y.Z] - 未推送`，不写日期；正式推送时补写日期、取消「未推送」标记并标注（当前），同时把版本号同步到`manifest.json`与`package.json` |
| 日期 | `YYYY-MM-DD`；未推送条目不写日期 |
| 排列 | 版本倒序，最新在上 |
